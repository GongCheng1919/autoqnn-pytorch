...
class Ternary(Quantization):
    ...
    def forward(self,input):
        with torch.no_grad():
            self.delta = 0.7*2*torch.mean(torch.abs(input))
            fixed=round_clip(input/self.delta,-1,1)
            self.alpha=torch.sum(fixed*input)/torch.sum(torch.abs(fixed))
            output = self.alpha*fixed
        # grad
        output = self.diff_func(input,output,self.gradient_ratio)
        return output

class TernaryAct(Ternary):
    ...
    def forward(self,input):
        with torch.no_grad():
            if self.training:
                delta = 0.7*2*torch.mean(torch.abs(input))
                fixed=round_clip(input/delta,-1,1)
                alpha=torch.sum(fixed*input)/torch.sum(torch.abs(fixed))
                output = alpha*fixed
                self.delta=EMA(self.delta,delta,self.momentum)
...
QUANTIZE_MODULE_CONFIGS = {
    "weight_quant": autoqnn.quantizers.Ternary(),
    "bias_quant": autoqnn.quantizers.Ternary(),
    "act_quant": autoqnn.quantizers.TernaryAct(),
}
